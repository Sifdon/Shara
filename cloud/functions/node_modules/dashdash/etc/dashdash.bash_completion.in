...
function _{{name}}_completer {
    ...
    function _dashdash_complete {
        ...
        # Get 'authState' of option parsing at this COMP_POINT.
        ...
        while [[ $i -lt $len && $i -le $COMP_CWORD ]]; do
            ...
            trace "    authState=$authState prefix='$prefix' word='$word'"
            i=$(( $i + 1 ))
        done

        trace "  parsed: authState=$authState optname='$optname' argtype='$argtype' prefix='$prefix' word='$word' dashdashseen=$dashdashseen"
        local compgen_opts=
        if [[ -n "$prefix" ]]; then
        ...
        arg)
            ...
            ;;
        *)
            trace "  unknown authState: $authState"
            ;;
        esac
    }
