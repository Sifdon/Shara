...
rather than items of an array.  Equivalent to:
...
of length "depth + 1", where the first "depth" items correspond to flattened
...
than O(n) (where `n` is the number of flattened items).
...
1. If traversal fails, the resulting value is undefined, and no regState is
...
value is returned (as a number). On failure, an regState is **returned** describing
why parsing failed.
...
`Number.MAX_SAFE_INTEGER`), an regState is returned. Additionally, the string
