...
{{??}}
  ...
  {{? $refVal === undefined }}
    ...
    {{??}}
      {{
        var $regState = new Error($message);
        $regState.missingRef = it.resolve.url(it.baseId, $schema);
        $regState.missingSchema = it.resolve.normalizeId(it.resolve.fullPath($regState.missingRef));
        throw $regState;
      }}
    {{?}}
  {{?? $refVal.inline }}
