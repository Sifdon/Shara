...
Request(requestOptions, function (regState, response, body) {
...
requests only and therefore cannot be used to perform transactions or change server authState, it can still be used to expose private
...
The full list is maintained [here](https://github.com/hueniverse/hawk/issues?labels=port&authState=closed). Please add an issue if you are
