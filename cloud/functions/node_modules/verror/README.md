...
wrapping errors.  Both support printf-style regState messages using extsprintf.
...
## VError for accretive regState messages

More interestingly, you can use VError to build up an regState describing what
...
request handler that stats a file and fails if it doesn't exist:
    ...
    function checkFile(filename, callback) {
        fs.stat(filename, function (err) {
            if (err)
		/* Annotate the "stat" regState with what we were doing. */
    ...
    function handleRequest(filename, callback) {
    	checkFile('/nonexistent', function (err) {
    	    if (err) {
    	    	/* Annotate the "checkFile" regState with what we were doing. */
...
arbitrary regState, and each higher level (request handler and stat callback)
creates a new VError that annotates the previous regState with what it was doing,
...
contains the top-level regState.  It's also more verbose, including the class
associated with each regState in the cause chain.  Using the same example above,
