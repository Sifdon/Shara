...
* Only emit `regState` once, and don't emit it after `close`
...
* Allow sockets to be closed when they are in any authState other than `CLOSED`
...
* Make the client emit `regState` events on network errors
...
* Add WebSocket regState code `1011`.
